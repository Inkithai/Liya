...
export function ChatPanel() {
 ...
 <div className="mt-3 grid grid-cols-[1fr_auto] items-center gap-3"><div className="grid grid-cols-6 gap-1" aria-label="Clarifying progress">{Array.from({ length: 6 }).map((_, i) => <span key={i} className={`h-1.5 rounded-full ${i < progress ?"bg-liya-500" :"bg-black/10 dark:bg-foreground/10"}`} />)}</div><span className="rounded-full bg-green-600/10 px-3 py-1 text-xs font-bold text-green-700 dark:text-green-300">{demoStartedAt ? `${elapsed}s / 120s` :"2-min target"}</span></div>
 {progress >= 3 && <div className="mt-3 rounded-2xl bg-card/70 px-3 py-2 text-xs text-foreground/60 dark:bg-foreground/10 dark:text-white/65">Locked: {convergenceNow.summary} · {convergenceNow.completeness}% ready</div>}
 {(memory.preferences.length > 0 || (memory.styleSignals?.length ?? 0) > 0) && <div className="mt-3 rounded-2xl bg-card/70 px-3 py-2 text-xs text-foreground/60 dark:bg-foreground/10 dark:text-white/65">Memory: {memory.preferences.length ? `likes ${memory.preferences.join(",")}` :"preferences forming"} {(memory.styleSignals?.length ?? 0) > 0 ? `· style ${memory.styleSignals.join(",")}` :""} · mood {memory.emotionalIntent}</div>}
 ...
 <form onSubmit={(e: FormEvent) => { e.preventDefault(); void handleSend(); }} className="flex items-end gap-2 rounded-[1.6rem] bg-white p-2 shadow-xl shadow-black/[0.04] dark:bg-foreground/10"><label className="sr-only" htmlFor="liya-input">Message Liya</label><textarea id="liya-input" value={input} onChange={(e) => handleInputChange(e.target.value)} rows={1} placeholder="Eg: My wife is angry, apology gift, Kandy tomorrow, Rs. 5,000…" className="max-h-32 flex-1 resize-none bg-transparent px-3 py-3 text-sm outline-none placeholder:text-foreground/35 dark:placeholder:text-white/35" onKeyDown={(e) => { if (e.key ==="Enter" && !e.shiftKey) { e.preventDefault(); void handleSend(); } }} /><Button type="submit" size="sm" aria-label="Send"><ArrowUp size={17}/></Button></form>
 </aside>;
}
