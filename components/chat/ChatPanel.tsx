...
export function ChatPanel() {
  ...
  return <aside className="glass flex h-[calc(100vh-6rem)] min-h-[620px] flex-col rounded-[2rem] p-4" aria-label="AI shopping conversation">
    ...
    {progress >= 3 && <div className="mt-3 rounded-2xl bg-white/70 px-3 py-2 text-xs text-black/60 dark:bg-white/10 dark:text-white/65">Locked: {convergenceNow.summary} · {convergenceNow.completeness}% ready</div>}
    {(memory.preferences.length > 0 || (memory.styleSignals?.length ?? 0) > 0) && <div className="mt-3 rounded-2xl bg-white/70 px-3 py-2 text-xs text-black/60 dark:bg-white/10 dark:text-white/65">Memory: {memory.preferences.length ? `likes ${memory.preferences.join(", ")}` : "preferences forming"} {(memory.styleSignals?.length ?? 0) > 0 ? `· style ${memory.styleSignals.join(", ")}` : ""} · mood {memory.emotionalIntent}</div>}
    ...
    <div className="mb-3 grid grid-cols-2 gap-2"><Button variant="secondary" size="sm" onClick={runDemo}><PlayCircle size={16}/> Demo Mode</Button><Button variant="secondary" size="sm" onClick={() => handleSend("Smart shopping planner for an occasion, budget, date and location") }><Wand2 size={16}/> Planner</Button><Button variant="secondary" size="sm" onClick={() => handleSend("Gift finder: ask me 5 guided questions") }><Gift size={16}/> Gift finder</Button><Button variant="secondary" size="sm" onClick={startVoice}>{listening ? <MicOff size={16}/> : <Mic size={16}/>} Voice</Button></div>
