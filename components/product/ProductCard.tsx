...
export function ProductCard({ product, context, featured = false }: { product: Product; context: ShopperContext; featured?: boolean }) {
  ...
  return <motion.article layout initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="group overflow-hidden rounded-[2rem] border border-black/5 bg-[hsl(var(--card))] shadow-xl shadow-black/[0.04] dark:border-white/10 dark:bg-white/[0.06]">
    <div className="relative aspect-[4/3] overflow-hidden bg-liya-100/50">
      ...
      <div className="absolute left-3 top-3 rounded-full bg-[hsl(var(--card))] px-3 py-1 text-xs font-semibold shadow-lg backdrop-blur border border-black/5 dark:border-white/10">{product.category ?? "Kapruka"}</div>
      <div className={`absolute bottom-3 left-3 rounded-full px-3 py-1 text-xs font-black shadow-lg backdrop-blur ${trust.tone === "green" ? "bg-green-600 text-white" : trust.tone === "amber" ? "bg-amber-500 text-white" : "bg-[hsl(var(--card))] text-ink border border-black/5 dark:border-white/10"}`}>{trust.label}</div>
      <button aria-label="Wishlist" onClick={() => { toggleWishlist(product); toast.success(wishlist ? "Removed from wishlist" : "Saved for later") }} className="focus-ring absolute right-3 top-3 grid h-10 w-10 place-items-center rounded-full bg-[hsl(var(--card))] shadow-lg backdrop-blur border border-black/5 dark:border-white/10"><Heart size={18} className={wishlist ? "fill-pink-500 text-pink-500" : ""}/></button>
    </div>
    <div className="p-4">
      ...
      <p className="mt-3 rounded-2xl bg-[hsl(var(--muted))] p-3 text-sm italic leading-5 text-[hsl(var(--foreground))] dark:bg-white/10 dark:text-white/70">"{emotionalLine}"</p>
