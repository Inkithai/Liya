...
export function ProductShelf({ loading = false }: { loading?: boolean }) {
 ...
 <div><p className="flex items-center gap-2 text-sm font-black text-liya-700 dark:text-liya-300"><Scale size={16}/> Liya’s honest comparison</p><p className="mt-1 text-sm text-foreground/60">{compared.length < 2 ?"One item selected — tap Compare on one more card to unlock the decision panel." :"Visible here so you don’t miss it: pick a contender, see why it wins or loses, then add with confidence."}</p></div>
