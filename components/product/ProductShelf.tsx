...
export function ProductShelf({ loading = false }: { loading?: boolean }) {
  ...
  return <section aria-label="Recommended products" className="space-y-5">
    <div className="flex items-end justify-between gap-4"><div><p className="flex items-center gap-2 text-sm font-semibold text-liya-700 dark:text-liya-300"><Sparkles size={16}/> Liya’s persistent shelf {loading ? "· refreshing quietly" : ""}</p><h2 className="text-2xl font-black tracking-tight">Recommendations with reasons</h2></div><p className="text-sm text-black/70 dark:text-white/55">{products.length} curated picks</p></div>
