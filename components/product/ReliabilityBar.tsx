...
export function ReliabilityBar() {
 ...
 <div className="mt-2 flex flex-wrap gap-2 text-[11px]"><span className="rounded-full bg-card/70 px-2.5 py-1 dark:bg-black/20">Decision Confidence: {decisionConfidence}%</span><span className="rounded-full bg-card/70 px-2.5 py-1 dark:bg-black/20">Delivery Confidence: {deliveryConfidence}</span><span className="rounded-full bg-card/70 px-2.5 py-1 dark:bg-black/20">{config.fallback}</span></div>
 </div>;
}
